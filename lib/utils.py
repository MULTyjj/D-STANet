...
def load_graphdata_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size,
    ...
    这个函数转为DSTANet，DSTANet设计，返回的数据x都是通过减均值除方差进行归一化的，y都是真实值
...
def compute_val_loss_DSTANet(net, val_loader, criterion, masked_flag, missing_value, sw, epoch, limit=None):
    """
...
def evaluate_on_test_DSTANet(net, test_loader, test_target_tensor, sw, epoch, _mean, _std):
    """
...
def predict_and_save_results_DSTANet(net, data_loader, data_target_tensor, global_step, metric_method, _mean, _std,
                                    params_path, type):
    """
    :param net: nn.Module
    ...
    print('all MAE: %.2f' % (min_mae))
    print('all RMSE: %.2f' % (min_rmse))
    print('all MAPE: %.2f' % (min_mape))

    excel_list.extend([min_mae, min_rmse, min_mape])
    print(excel_list)
